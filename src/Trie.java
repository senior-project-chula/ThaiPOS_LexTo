...
//	postype = type from orchid (second column in dict)
//	typeforstock = type declare for stock (third column in dict)
//	public boolean add(String s) {
//		return add(s, 0);
//	}
//	public boolean add(String s,int postype) {
//		return add(s, 0,postype);
//	}
	public boolean add(String s,int postype,int typeforstock) {
		return add(s, 0,postype, typeforstock);
	}
...
//		return true;
//	}
	private boolean add(String s, int index,int postype,int typeforstock) {
		if(index==s.length()) {
			if(isWord)
				return false;
			isWord=true;
			pos=postype;
			stockpos=typeforstock;
			return true;
		}
		char c=s.charAt(index);
		for (int i=0; i<numChildren; i++)
			if (child[i].ch == c)
				return child[i].add(s, index + 1,postype,typeforstock);
	...
	public int[] containsPOS(String s) {
		Trie t=getNode(s);
		if (t==null)
			return new int[]{-1,10};
		if(t.isWord){
			return new int[]{1,t.pos};
		}
		else
			return new int[]{0,10};
	}
	public int[] containsStockPOS(String s) {
		Trie t=getNode(s);
		if (t==null)
			return new int[]{-1,10,100};
		if(t.isWord){
			return new int[]{1,t.pos,t.stockpos};
		}
		else
